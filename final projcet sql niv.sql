...
-- 3.1 Cumulative returns per stock
CREATE VIEW stocks_cumulative_returns AS
SELECT
  sp.stock_id,
  s.Ticker,
  ROUND((
    LAST_VALUE(sp.close_price) OVER (PARTITION BY sp.stock_id ORDER BY sp.trade_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) -
    FIRST_VALUE(sp.close_price) OVER (PARTITION BY sp.stock_id ORDER BY sp.trade_date)
  ) /
  FIRST_VALUE(sp.close_price) OVER (PARTITION BY sp.stock_id ORDER BY sp.trade_date) * 100, 2) AS total_return_pct
FROM stock_prices sp
JOIN stocks s ON sp.stock_id = s.id;

-- 3.2 Cumulative return for S&P 500
CREATE VIEW sp500_cumulative_return AS
SELECT
  ROUND((
...
CREATE VIEW sp500_sharpe_ratio AS
SELECT 
  'S&P 500' AS Ticker,
...
CREATE VIEW stocks_sharpe_ratio AS
...
CREATE VIEW sharpe_ratio_comparison AS
...
CREATE VIEW total_returns_comparison AS
